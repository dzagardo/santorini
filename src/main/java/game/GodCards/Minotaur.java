...
/**
 * Your Worker may move into an opponent Workerʼs space, if their Worker can be forced
...
public class Minotaur implements GodCard {
    ...
    private boolean validateMinotaurMove(Board board, Worker w, int x, int y) {
    ...
    public Game move(int x, int y, int pNum, int wNum, Game game, Player p) {
        Player minotaur = game.getPlayerHelper(game.getTurn());
        Board board = game.getBoard();
        ...
        // if normal move
        if (p.getPlayerNumber() == game.getTurn()) {
            if (p.getOnDeck() == null) {
                return game;
            }
        ...
        // if minotaur move
        else if (p.getPlayerNumber() != game.getTurn()) {
            Worker workerMinotaur = minotaur.getOnDeck();
            Worker workerVictim = p.getOnDeck();
            int targetX = x;
            int targetY = y;
            int workerVictimX = targetX - workerMinotaur.getWorkerX() + workerVictim.getWorkerX();
            int workerVictimY = targetY - workerMinotaur.getWorkerY() + workerVictim.getWorkerY();
            int turn = game.getTurn();
            int[][] movePos = new int[1][2];
            movePos[0][0] = targetX;
            movePos[0][1] = targetY;
            if (!validateMovePos(board, workerMinotaur, targetX, targetY, turn)) {
                return game;
            }
            if (!validateMinotaurMove(board, workerVictim, workerVictimX, workerVictimY)) {
                return game;
            }
            // set position for victim
            board.getSpace(p.workerGetPosHelper(workerVictim)).vacate();
            workerVictim = p.workerPosSetter(workerVictim, workerVictimX, workerVictimY);
            board.getSpace(p.workerGetPosHelper(workerVictim)).occupy(workerVictim);
            // set position for minotaur
            board.getSpace(minotaur.workerGetPosHelper(workerMinotaur)).vacate();
            workerMinotaur = minotaur.workerPosSetter(workerMinotaur, targetX, targetY);
            board.getSpace(minotaur.workerGetPosHelper(workerMinotaur)).occupy(workerMinotaur);
            if (board.getSpace(targetX, targetY).getTowerHeight() == WIN_CONDITION) {
                game.setGameOver();
                game.endGame(minotaur);
            }
            minotaur.hasMoved();
            workerMinotaur.hasMoved();
        }
        return new Game(board, game.getHistory());
    }
    ...
    public String godRules() {
        return "Minotaur: Your Worker may move into an opponent " +
                "Workerʼs space, if their Worker can be forced " +
                "one space straight backwards to an unoccupied " +
                "space at any level.";
    }
}
