...
/**
 Apollo: Your Worker may move into an opponent Workerʼs space by forcing their Worker to the space
 yours just vacated.
 */
public class Apollo implements GodCard {
    ...
    public Game move(int x, int y, int pNum, int wNum, Game game, Player p) {
        Player apollo = game.getPlayerHelper(game.getTurn());
        Board board = game.getBoard();
        ...
        // if minotaur move
            ...
            Worker workerApollo = apollo.getOnDeck();
            Worker workerVictim = p.getOnDeck();
            int targetX = x;
            int targetY = y;
            int workerVictimX = workerApollo.getWorkerX();
            int workerVictimY = workerApollo.getWorkerY();
            int turn = game.getTurn();
            int[][] movePos = new int[1][2];
            movePos[0][0] = targetX;
            movePos[0][1] = targetY;
            if (!validateMovePos(board, workerApollo, targetX, targetY, turn)) {
                return game;
            }
            board.getSpace(p.workerGetPosHelper(workerVictim)).vacate();
            board.getSpace(apollo.workerGetPosHelper(workerApollo)).vacate();
            workerApollo = apollo.workerPosSetter(workerApollo, targetX, targetY);
            board.getSpace(apollo.workerGetPosHelper(workerApollo)).occupy(workerApollo);
    ...
    public String godRules() {
        return "Apollo: Your Worker may move into " +
                "an opponent Workerʼs space by " +
                "forcing their Worker to the space " +
                "yours just vacated.";
    }
}
